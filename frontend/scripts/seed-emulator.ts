...
async function seedDatabase() {
  ...
  // that takes it.
  //
  // Every entry carries an `adminType`, which keeps all three out of the
  // categorisation queue at /admin/notatki/kategoryzacja - that queue asks for
  // `adminType: "none"`, and its spec works through the two entries it seeds
  // itself and then waits for "Wszystkie notatki skategoryzowane!". Fixtures
  // sitting there untyped would leave the queue non-empty forever and that
  // message would never come. `adminType` is admin bookkeeping and is not
  // drawn on an entity page, so the pictures below are unaffected by it.
  const seededNotes = {
    "1_seed-notes-author": {
      nodeId: "1",
